    ...
    if not params.no_ref_sim:
        ...
        for i, (q_acc, q_seq) in enumerate(sorted_reference_tuples):
            ...
            for j, (r_acc, r_seq) in enumerate(sorted_reference_tuples):
                ...
                #     continue
                t=10000
                ed = edlib_ed(q_seq, r_seq, mode="NW", task="distance",k=t)# k=2*max(len(q_seq), len(r_seq) ) )
...
def get_minimizers_2set_simple(querys, targets):
    ...
    for acc1, seq1 in querys.items():
        ...
        for acc2, seq2 in targets.items():
                ...
                edit_distance = edlib_ed(seq1, seq2, mode="NW", k=best_ed)#k = 2*len(seq1)) # seq1 = query, seq2 = target
...
def get_alignments(best_edit_distances, querys, targets):
    ...
    print("BED", best_edit_distances)
    for acc1 in best_edit_distances:
        seq1 = querys[acc1]
        best_ed = 100000000000#len(seq1)
        ...
        for acc2 in best_edit_distances[acc1]:
            ...
            if sw_ed < best_ed:
                ...
                print("HERE",acc1,acc2)
            # seq1_aln, match_line, seq2_aln = result.alignment
    #print(best_edit_distances_parasail)
...
def get_best_match(corrected_reads, reference_transcripts, params,outfile):
    ...
    file_id_list=file_id.split("_")[1:]
    print(file_id_list)
    new_id=list_to_file_name(file_id_list)
    print("newID",new_id)
    #error_file_name="error_"+new_id+".csv"
    #print(error_file_name)
    #error_file=os.path.join(out_dir,error_file_name)
    #print("getting_best match")
    print("OUTFILE",outfile)
    outfile_id="results_"+new_id+".csv"
    outfile_name=os.path.join(out_dir,outfile_id)
    print(outfile_name)
    ...
    print("IRT",reference_transcripts)
    ...
    #print(error_types_container)
    #print("ERRORFILE")
    #for acc,error_types in error_types_container.items():
    #    errorfile.write("{0},{1},{2},{3},{4}\n".format(acc,best_match_container[q_acc],error_types[0],error_types[1],error_types[2]))
    #errorfile.close()
...
def main(args):
    ...
    print("IIRT",reference_transcripts)
    ...
    """if not args.no_ref_sim:
        ...
        if len(ref_similarities) % 2 == 0:
            median_error = (ref_similarities[int(len(ref_similarities) / 2) - 1] + ref_similarities[
                int(len(ref_similarities) / 2)]) / 2.0
        else:
