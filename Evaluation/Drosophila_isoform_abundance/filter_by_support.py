...
def read_supp_file(full_path_support,supp_dict):
    supp_file = open(full_path_support, 'r')
    Lines = supp_file.readlines()
    for line in Lines:
        split = line.split(":")
        supp_id = split[0]
        supp_supp = int(split[1])
        supp_dict[supp_id] = supp_supp
def filter_and_write(full_outpath,supp_dict,ab,transcriptome):
...
def main(args):
    ...
    print(path)
    abundance_list=[1,2,3,5,10,15,20,25,50]
    #unique_cl_ids = set()
    supp_dict={}
    #for entry in os.listdir(path):
    #    if entry.endswith('.fastq'):
    #        name = entry.split(".")[0]
    #        id=name.split("_")[-1]
    #        unique_cl_ids.add(id)
        #print(entry)
    #for u_id in unique_cl_ids:
    transcriptome_name='isON_transcriptome.fastq'
    support_name='isON_transcriptome_supp.txt'
    full_path_support=os.path.join(path,support_name)
    read_supp_file(full_path_support,supp_dict)
    ...
    for ab in abundance_list:
        outfile_name="transcriptome_"+str(ab)+".fastq"
        full_outpath=os.path.join(outfolder,outfile_name)
        filter_and_write(full_outpath,supp_dict,ab,transcriptome)
