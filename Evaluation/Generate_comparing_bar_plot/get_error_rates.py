...
#TODO: change acc so that we get rid of the number of supporting reads from the variable
...
def read_mappingfile(mappingfile):
    accession_nreads = open(mappingfile, 'r')
    entries = accession_nreads.readlines()
    clean_entries = [sub.replace('\n', '') for sub in entries]
    cleaner_entries=[sub.replace('>','')for sub in clean_entries]
    ids=cleaner_entries[::2]
...
def main(args):
    ...
    new_error_rates_per_read={}
    for acc in error_rates_per_read:
        newacc=acc.replace(",","")
        new_error_rates_per_read[newacc]=error_rates_per_read[acc]
    print(new_error_rates_per_read)
    for acc in new_error_rates_per_read:
        nr_supp=mapping_dict[acc]
        reference_name, errors, insertions, deletions, subs, matches, err_rate = new_error_rates_per_read[acc]
        outfile.write(
