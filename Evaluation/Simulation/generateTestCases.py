...
def generate_coords(n):
    coordinates=[]
    pos=0
    #coord_len_pool=[10, 20, 50,100,200]
    coord_len_pool=[200]
    coordinates.append(pos)
    for i in range(n+1):
        next_dist=random.choice(coord_len_pool)
        #print("ND",next_dist)
        pos+=next_dist
        coordinates.append(pos)
    return coordinates
def generate_isoforms_def_start_end(args, ref_path):
    ...
    ref = {acc: seq for acc, (seq, qual) in readfq(open(ref_path, 'r'))}
    seq = ref[list(ref.keys())[0]]
    coordinates=generate_coords(len(args.probs))
    #print(coordinates)
    exon_coords = [(start, stop) for start, stop in zip(coordinates[:-1], coordinates[1:])]
    exons = [seq[j_start: j_stop] for (j_start, j_stop) in exon_coords]
    #print("Exons")
    #print(exons)
    isoforms={}
    ...
    while actual_isoforms < args.n_isoforms:
        ...
        if not isoform in known_isoforms or not isoform:
            ...
            if args.read_dist=="uniform":
                ...
                double_seed=random.choice([1,2,4,8,16,32,64,128])
            #TODO change this part. We always want isoforms to be supported by 1-10 reads
            #if double_seed>1:
...
if __name__ == '__main__':
   ...
   # parser.add_argument('--isoforms', type=str,
    ...
    parser.add_argument('--read_dist',type=str,help="type of read distribution: either uniform (20 reads per isoform) or exp (2^(x-2)|x<=10)")
